...
public class roleAuthenticationFilter implements Filter {
    ...
    private void doBeforeProcessing(ServletRequest request, ServletResponse response)
        ...
        String[] roleList = {"user", "admin"}; //role 0 | 1
        String role = null;
        try {
            HttpSession session = httpRequest.getSession();
            Account account = (Account) session.getAttribute("account");
            role = roleList[account.getRole()]; // user

        } catch (Exception e) {
        }
        String path = httpRequest.getServletPath();
        boolean isAdminPage = path.startsWith("/admin");
        boolean isUserPage = path.startsWith("/user");
        boolean isComponentsPage = path.startsWith("/components");
        if (isAdminPage || isUserPage || isComponentsPage) {
            if (!path.startsWith("/" + role) || isComponentsPage) {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (debug) {
            log("roleAuthenticationFilter:doFilter()");
        }
